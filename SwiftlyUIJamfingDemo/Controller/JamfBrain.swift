...
//class JamfBrain: ObservableObject {
    ...
    @Published var computers: [Computer] = []
        @Published var allComputers: Computer = Computer(from: [Computer]() as! Decoder)
...
//        @Published var allComputersBasicDict = [ComputerBasicRecord: ComputerBasicRecord]()
        ...
        @Published var allComputersDict = [Computer]()
...
//    }
    ...
    func getComputers(server: String) async throws {
        ...
        request.setValue("application/json", forHTTPHeaderField: "Accept")
        separationLine()
        print("Running func: getComputers")
        print("jamfURLQuery is: \(jamfURLQuery)")
...
//        DispatchQueue.main.async {
//            
//            self.allComputersDict = self.allComputers
//        }
...
//    }
        ...
        // This function queries the Jamf Pro API for a authentication token
        func getToken(server: String, user: String, password: String) async throws {
            print("Getting token")
            
            guard let base64 = encodeBase64(user: user, password: password) else {
                print("Error encoding user/password")
